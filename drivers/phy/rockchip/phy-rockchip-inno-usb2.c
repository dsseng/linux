...
static int rk3562_usb2phy_tuning(struct rockchip_usb2phy *rphy)
{
	...
	regmap_clear_bits(rphy->phy_base, 0x0030, BIT(2));
	regmap_clear_bits(rphy->phy_base, 0x0430, BIT(2));

	/* Enable pre-emphasis during non-chirp phase */
	regmap_update_bits(rphy->phy_base, 0x0000, GENMASK(2, 0), 0x04);
	regmap_update_bits(rphy->phy_base, 0x0400, GENMASK(2, 0), 0x04);

	/* Set HS eye height to 425mv(default is 400mv) */
	regmap_update_bits(rphy->phy_base, 0x0030, GENMASK(6, 4), (0x05 << 4));
	regmap_update_bits(rphy->phy_base, 0x0430, GENMASK(6, 4), (0x05 << 4));
...
static int rk3568_usb2phy_tuning(struct rockchip_usb2phy *rphy)
{
	...
	/* Turn off differential receiver by default to save power */
	regmap_clear_bits(rphy->phy_base, 0x30, BIT(2));
	...
	if (rphy->phy_cfg->reg == 0xfe8a0000) {
		...
		regmap_update_bits(rphy->phy_base, 0x30, GENMASK(6, 4), (0x06 << 4));
...
static int rv1106_usb2phy_tuning(struct rockchip_usb2phy *rphy)
{
	...
	if (version) {
		...
		regmap_update_bits(rphy->phy_base, 0x40, GENMASK(5, 3), (0x03 << 3));
	}

	/* Set RX Squelch trigger point configure to 4'b0000(112.5 mV) */
	regmap_update_bits(rphy->phy_base, 0x64, GENMASK(6, 3), (0x00 << 3));

	/* Turn off differential receiver by default to save power */
	regmap_clear_bits(rphy->phy_base, 0x100, BIT(6));

	/* Set 45ohm HS ODT value to 5'b10111 to increase driver strength */
	regmap_update_bits(rphy->phy_base, 0x11c, GENMASK(4, 0), 0x17);

	/* Set Tx HS eye height tuning to 3'b011(462 mV)*/
	regmap_update_bits(rphy->phy_base, 0x124, GENMASK(4, 2), (0x03 << 2));

	/* Bypass Squelch detector calibration */
	regmap_update_bits(rphy->phy_base, 0x1a4, GENMASK(7, 4), (0x01 << 4));
	regmap_update_bits(rphy->phy_base, 0x1b4, GENMASK(7, 4), (0x01 << 4));

	/* Set HS disconnect detect mode to single ended detect mode */
	regmap_set_bits(rphy->phy_base, 0x70, BIT(2));
